...
test('approved Pumpio preview assets and high-resolution V4 masters exist', async () => {
  ...
  for (const source of sources) {
    ...
    const minimumSize = source.includes('/v4/') ? 40_000 : 100_000;
  ...
  for (const name of ['pumpio-dealmaker.png', 'pumpio-analyst.png', 'pumpio-floor-trader.png']) {
    const master = path.resolve(root, '..', 'art', 'pumpios-v4', 'masters', name);
